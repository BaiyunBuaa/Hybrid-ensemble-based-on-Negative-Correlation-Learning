# -*- coding: utf-8 -*-
"""
Created on Thu Nov 19 15:52:03 2020
...
grid_search_1 = GridSearchCV(reg_1, param_grid_1, cv=5, n_jobs=n_jobs,verbose=1)    #网格搜索+交叉验证
...
grid_search_2 = GridSearchCV(reg_2, param_grid_2, cv=5, n_jobs=n_jobs,verbose=1)   #网格搜索+交叉验证
...
grid_search_3 = GridSearchCV(reg_3, param_grid_3, cv=5, n_jobs=n_jobs,verbose=1)   #网格搜索+交叉验证
...
grid_search_4 = GridSearchCV(reg_4, param_grid_4, cv=5, n_jobs=n_jobs,verbose=1)   #网格搜索+交叉验证
...
grid_search_5 = GridSearchCV(reg_5, param_grid_5, cv=5, n_jobs=n_jobs,verbose=1)   #网格搜索+交叉验证
...
grid_search_7 = GridSearchCV(reg_7, param_grid_7, cv=5, n_jobs=n_jobs,verbose=1)
...
grid_search_8 = GridSearchCV(reg_8, param_grid_8, cv=5, n_jobs=n_jobs,verbose=1)
...
grid_search_9 = GridSearchCV(reg_9, param_grid_9, cv=5, n_jobs=n_jobs,verbose=1)
...
grid_search_11 = GridSearchCV(reg_11, param_grid_11, cv=5, n_jobs=n_jobs,verbose=1)
...
reg_12 = MLPRegressor(max_iter=5000)
grid_search_12 = GridSearchCV(reg_12, param_grid_12, cv=5, n_jobs=n_jobs,verbose=10)
...
grid_search_6 = GridSearchCV(PolynomialRegression(), param_grid_6, cv=5, n_jobs=n_jobs,verbose=10)

grid_search_6.fit(x_train_sta, y_train_sta)
y_pre_6 = grid_search_6.predict(x_test_sta)

for i in range(len(y_pre_6)):
    if y_pre_6[i] > 5:
        y_pre_6[i] = 5
    if y_pre_6[i] <-5:
        y_pre_6[i] = -5
