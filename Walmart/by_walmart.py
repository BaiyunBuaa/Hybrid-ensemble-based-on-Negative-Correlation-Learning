...
#顺序变量做标准化，名义变量不用做标准化
dataset = pd.read_csv('C:/Users/XEON-B00/Desktop/baiyun/Walmart/Walmart_Store_sales.csv')

x = dataset[['Store','Holiday_Flag','Temperature','Fuel_Price','CPI','Unemployment']]
...
#处理名义变量    
df = pd.get_dummies(x, columns = ['Store'], dummy_na = False)
original_column = list(x.columns)
new_column = [c for c in df.columns if c not in original_column ]

root = 'C:/Users/XEON-B00/Desktop/baiyun/Walmart/pkl_data/'
