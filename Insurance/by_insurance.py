...
#顺序变量做标准化，名义变量不用做标准化
dataset = pd.read_csv('C:/Users/XEON-B00/Desktop/baiyun/Insurance/insurance.csv')
...
#处理名义变量    
df = pd.get_dummies(x, columns = ['sex','smoker','region'], dummy_na = False)
original_column = list(x.columns)
new_column = [c for c in df.columns if c not in original_column ]

root = 'C:/Users/XEON-B00/Desktop/baiyun/Insurance/pkl_data/'
...
scaler_x.fit(x_train[['age','bmi','children']])
x1 = scaler_x.transform(x_train[['age','bmi','children']])
x_train['age'],x_train['bmi'],x_train['children'] = x1[:,0],x1[:,1],x1[:,2]
x_train_sta = x_train
x2 = scaler_x.transform(x_test[['age','bmi','children']])
